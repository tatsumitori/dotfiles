-- キーバインドを表示する（wezterm show-keys）
-- SUPERはWindowsならWindowsキー、Macならcommandキーで、ALTはWindowsならaltキーでMacならoptionキー
...
function M.setup(wezterm, config)
	...
	config.keys = {
		...
		-- { key = "Enter", mods = "SHIFT", action = act{SendString="\x1b\r"}},  -- Claude Code での複数行入力のため、Shift + Enter で改行シーケンスを送信
		...
		{ key = "0", mods = "ALT|CTRL", action = act.ActivateTab(9) },
